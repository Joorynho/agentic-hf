        ...
        if order.side == Side.BUY:
            ...
            if accountant:
                ...
                if not accountant.has_sufficient_cash(required_notional):
                    ...
                    if available <= 1.0:
                        logger.warning(
                            "[equities.exec] Order %s rejected: insufficient cash ($%.2f available, $%.2f required)",
                    ...
                    if scaled_qty * est_price < 1.0:
                        logger.warning(
                            "[equities.exec] Order %s rejected: scaled qty too small ($%.2f cash)",
                            order.id, available,
                        )
                        return {
                            "execution_rejected": True,
                    ...
                    logger.info(
                        "[equities.exec] Scaling order %s from %.4f to %.4f (cash limit: $%.2f)",
