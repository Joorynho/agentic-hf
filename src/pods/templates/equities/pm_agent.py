...
max_hold_days: optional. Set to a specific number of days if the thesis is time-bound (e.g. earnings catalyst in 14 days -> max_hold_days: 20). Set to 0 (default) for thesis-driven holds with no fixed time limit. Only set a limit when the trade thesis has a clear expiration.
...
class EquitiesPMAgent(BasePodAgent):
    ...
    def _apply_sizing_discipline(
        ...
        Conviction-based position caps (enforced, not advisory):
          conviction < 0.3  → max  5% NAV  (low confidence = small bet)
          conviction 0.3-0.5 → max 10% NAV
          conviction 0.5-0.7 → max 15% NAV
          conviction > 0.7  → max 20% NAV  (high confidence = full allocation)
        """
        ...
        # Conviction-scaled position cap (replaces the flat PM_MAX_SINGLE_TRADE_PCT)
        ...
        if conviction < 0.3:
            ...
            conv_cap = PM_MAX_SINGLE_TRADE_PCT  # 0.20
