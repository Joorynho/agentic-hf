...
_EQUITIES_SYSTEM = """You are an equity portfolio manager at a systematic macro hedge fund.
You receive real macro data, prediction market odds, and news headlines every cycle.
Your job is to synthesize ALL of this information into disciplined trading decisions.
...
2. PREDICTION MARKETS: Read the Polymarket questions and probabilities.
   - These are real-money prediction odds on policy, elections, rates, geopolitics.
   - High-probability events should inform positioning (e.g. rate cut likely = long growth).
3. NEWS: Read the headlines for catalysts, risks, and sentiment shifts.
4. POSITIONS: Check what you already hold to avoid doubling down or concentration risk.

Rules:
- Only trade if you have a clear thesis backed by the data.
- HOLD (empty trades) is the right call most of the time.
- Max 3 trades per cycle. Each must have a clear reasoning tied to specific data points.
- Use ETFs for broad views (SPY, QQQ, XLF, TLT) and single names for high-conviction ideas.
    ...
    async def _llm_decision(self, features: dict) -> dict:
        ...
        try:
            ...
            for t in trades[:3]:
                ...
                orders.append(order)
                logger.info("[equities.pm] LLM: %s %s %.0f (%s)", action, symbol, qty, reasoning)
