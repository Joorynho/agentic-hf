...
MAX_POSITION_PCT = 0.10  # max 10% of NAV per position
MAX_LEVERAGE = 2.0

class EquitiesRiskAgent(BasePodAgent):
    """Position-level risk checks for equities."""
    ...
    async def run_cycle(self, context: dict) -> dict:
        ...
        if accountant:
            ...
            if nav > 0 and new_notional / nav > MAX_POSITION_PCT:
                ...
                if max_qty <= 0:
                    logger.info("[equities.risk] Position limit hit for %s", order.symbol)
                    return {}
                revised = Order(
            ...
            # Leverage check: reject if gross leverage would exceed MAX_LEVERAGE
            total_notional = sum(
            ...
            if nav > 0 and projected_notional / nav > MAX_LEVERAGE:
                ...
                if max_add <= 0:
                    logger.info("[equities.risk] Leverage limit hit for %s", order.symbol)
                    return {}
