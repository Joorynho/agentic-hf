...
class EpsilonExecutionTrader(BasePodAgent):
    ...
    async def run_cycle(self, context: dict) -> dict:
        ...
        if risk_halt:
            ...
            logger.error(
                "[epsilon.exec] Order %s rejected: Risk halt active (%s)", order.id, risk_halt_reason
            )
        ...
        try:
            ...
            if mandate and self._pod_id in mandate.pod_allocations:
                ...
                if current_notional + requested_notional > max_notional:
                    ...
                    if available_notional <= 0:
                        logger.warning(
                            "[epsilon.exec] Order %s rejected: allocation limit reached (%.0f%%, current=$%.2f, max=$%.2f)",
                    ...
                    logger.info(
                        "[epsilon.exec] Scaling order %s from %.2f to %.2f (allocation limit)",
            ...
            if token and hasattr(token, "constraints") and token.constraints:
                ...
                if new_leverage > max_leverage:
                    logger.warning(
                        "[epsilon.exec] Order %s rejected: leverage limit (%.1fx) exceeded (current+requested=%.1fx)",
            ...
            logger.error("[epsilon.exec] Error checking governance constraints: %s", e)
        ...
        try:
            ...
            if result.status in ("FILLED", "PARTIAL"):
                ...
                try:
                    ...
                    logger.debug("[epsilon.exec] Failed to publish fill event: %s", e)
