...
from src.core.config.universes import FX_SEED
...
class FXResearcher(BasePodAgent):
    """Fetches macro data, Polymarket odds, news, and live prices for FX universe."""
    ...
    async def _review_universe(self, fred_snapshot, poly_signals, news_items, live_quotes) -> list[str]:
        """Use LLM to review and potentially update the FX universe daily."""
        try:
            from src.core.llm import llm_chat
        except Exception:
            return list(FX_SEED)

        current = self.recall("universe") or list(FX_SEED)
        ...
        prompt = (
            f"You are the FX researcher for a macro hedge fund.\n"
            f"Current universe ({len(current)} symbols): {', '.join(current[:30])}\n"
            f"Seed has {len(FX_SEED)} symbols (currency + country ETFs).\n\n"
            f"Context:\n- Headlines: {'; '.join(headlines[:8]) if headlines else 'none'}\n"
            f"- Movers: {'; '.join(movers[:6]) if movers else 'none'}\n"
            f"- FRED: DXY={fred_snapshot.get('DTWEXBGS', '?')}, 10Y={fred_snapshot.get('DGS10', '?')}\n\n"
        ...
        try:
            ...
            if start >= 0 and end > start:
                ...
                seed_kept = len([s for s in FX_SEED if s in new_universe])
                if seed_kept < len(FX_SEED) * _MIN_SEED_RETENTION:
                    return current
            ...
            logger.info("[fx.researcher] Universe review LLM failed: %s", e)
        return current

    async def run_cycle(self, context: dict) -> dict:
        current_universe = self.recall("universe") or list(FX_SEED)
        ...
        if not fred_snapshot and self.fred_adapter:
            try:
                ...
                logger.info("[fx.researcher] FRED failed: %s", e)
        ...
        if shared_poly is not None:
            ...
            try:
                ...
                logger.info("[fx.researcher] Polymarket failed: %s", e)
        ...
        if shared_news is not None:
            ...
            try:
                ...
                logger.info("[fx.researcher] RSS failed: %s", e)
        ...
        if shared_x is not None:
            ...
            try:
                ...
                logger.info("[fx.researcher] News feed failed: %s", e)
        self.store("x_feed", x_feed)
        self.store("x_tweet_count", len(x_feed))

        # Live price quotes for top FX instruments
        live_quotes: dict = {}
        if self.price_service:
            try:
                sample = FX_SEED[:_PRICE_SAMPLE_SIZE]
                live_quotes = await self.price_service.get_quotes(sample)
                logger.info("[fx.researcher] Live quotes: %d/%d symbols", len(live_quotes), len(sample))
            except Exception as e:
                logger.info("[fx.researcher] Live price fetch failed: %s", e)
        ...
        scored_headlines, scored_preds = score_items(headline_dicts, pred_dicts, "fx")
        ...
        logger.info("[fx.researcher] fred=%.3f poly=%.3f sentiment=%.3f -> macro=%.3f | news=%d x=%d prices=%d",
        ...
        try:
            ...
            for h in candidates[:2]:
                ...
                results = await self._web_searcher.search(
                    f"{title} forex currency analysis {now.strftime('%B %Y')}", max_results=3
                )
                if results:
                    top = results[0]
            ...
            if deep_dives:
                ...
                logger.info("[fx.researcher] %d headline deep-dives fetched", len(deep_dives))

            # Macro outlook search
            macro_outlook = self._ns.get("features", {}).get("macro_outlook", "") if self._ns.get("features") else ""
            if macro_outlook:
                results = await self._web_searcher.search(f"forex currency markets {macro_outlook} outlook {now.strftime('%B %Y')}")
                self._ns.set("web_search_results", results)
        except Exception as e:
            logger.debug("[fx.researcher] Web search skipped: %s", e)
        ...
        try:
            ...
            logger.debug("[fx.researcher] Events calendar skipped: %s", e)
