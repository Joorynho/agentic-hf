...
class FXExecutionTrader(BasePodAgent):
    ...
    async def run_cycle(self, context: dict) -> dict:
        ...
        if token is None or not token.is_valid():
            logger.warning(
                "[fx.exec] Invalid/expired token — order %s rejected", order.id
            )
        ...
        if risk_halt:
            ...
            logger.error(
                "[fx.exec] Order %s rejected: Risk halt active (%s)", order.id, risk_halt_reason
            )
        ...
        if drawdown_halt and order.side == Side.BUY:
            logger.error("[fx.exec] Order %s rejected: firm drawdown halt (no new buys)", order.id)
            return {
                "execution_rejected": True,
    ...
    async def _execute_via_alpaca(
        ...
        try:
            ...
            if mandate and self._pod_id in mandate.pod_allocations:
                ...
                if current_notional + requested_notional > max_notional:
                    ...
                    if available_notional <= 0:
                        logger.warning(
                            "[fx.exec] Order %s rejected: allocation limit reached (%.0f%%, current=$%.2f, max=$%.2f)",
                    ...
                    logger.info(
                        "[fx.exec] Scaling order %s from %.2f to %.2f (allocation limit)",
            ...
            if token and hasattr(token, "constraints") and token.constraints:
                ...
                if new_leverage > max_leverage:
                    logger.warning(
                        "[fx.exec] Order %s rejected: leverage limit (%.1fx) exceeded (current+requested=%.1fx)",
            ...
            logger.error("[fx.exec] Error checking governance constraints: %s", e)
        ...
        if order.side == Side.BUY:
            ...
            if accountant:
                ...
                if not accountant.has_sufficient_cash(required_notional):
                    ...
                    if available <= 1.0:
                        logger.warning(
                            "[fx.exec] Order %s rejected: insufficient cash ($%.2f available, $%.2f required)",
                    ...
                    if scaled_qty * est_price < 1.0:
                        logger.warning(
                            "[fx.exec] Order %s rejected: scaled qty too small ($%.2f cash)",
                            order.id, available,
                        )
                        return {
                            "execution_rejected": True,
                    ...
                    logger.info(
                        "[fx.exec] Scaling order %s from %.4f to %.4f (cash limit: $%.2f)",
        ...
        try:
            logger.info(
                "[fx.exec] Submitting order to Alpaca: %s %.0f %s @ $%s",
            ...
            if result.status in ("FILLED", "PARTIAL"):
                ...
                try:
                    ...
                    logger.debug("[fx.exec] Failed to publish fill event: %s", e)
            ...
            logger.error("[fx.exec] Order execution failed: %s", exc)
            return {
    ...
    def _queue_for_paper_adapter(self, order: Order) -> dict:
        ...
        logger.info(
            "[fx.exec] Order queued (paper mode): %s %.0f %s",
