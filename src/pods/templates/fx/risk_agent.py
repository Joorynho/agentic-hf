...
MAX_POSITION_PCT = 0.15  # FX ETFs are lower vol, allow larger positions
MAX_LEVERAGE = 2.0
    ...
    async def run_cycle(self, context: dict) -> dict:
        ...
        if accountant:
            ...
            if nav > 0 and new_notional / nav > MAX_POSITION_PCT:
                ...
                if max_qty <= 0:
                    logger.info("[fx.risk] Position limit hit for %s", order.symbol)
                    return {}
                revised = Order(
            ...
            if nav > 0 and projected_notional / nav > MAX_LEVERAGE:
                ...
                if max_add <= 0:
                    logger.info("[fx.risk] Leverage limit hit for %s", order.symbol)
                    return {}
