...
class DeltaExecutionTrader(BasePodAgent):
    ...
    async def _execute_via_alpaca(
        ...
        try:
            ...
            logger.error("[delta.exec] Order execution failed: %s", exc)
    ...
    def _queue_for_paper_adapter(self, order: Order) -> dict:
        ...
        logger.info("[delta.exec] Queued: %s %s %s", order.side.value, order.quantity, order.symbol)
