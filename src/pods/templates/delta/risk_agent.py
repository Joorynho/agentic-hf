...
MAX_NAME_PCT = 0.05  # max 5% per name
MAX_LEVERAGE = 1.0


class DeltaRiskAgent(BasePodAgent):
    """Max 5% per name, 1.0x leverage. Issues RiskApprovalToken."""
    ...
    async def run_cycle(self, context: dict) -> dict:
        ...
        if new_exp > MAX_NAME_PCT:
            revised_qty = max(1.0, order.quantity * (MAX_NAME_PCT / max(new_exp, 1e-8)))
            revised = Order(
            ...
            logger.info("[delta.risk] qty revised %.0f→%.0f", order.quantity, revised_qty)
