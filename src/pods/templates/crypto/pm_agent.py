...
WEB SEARCH:
If you need information not provided (e.g. a protocol upgrade date, regulatory news),
include "search_queries": ["query1", "query2"] (max 2). Results will be provided before your final decision.

Output JSON:
{"trades": [{"action": "BUY"|"SELL", "symbol": "XXX/USD", "qty": N, "conviction": 0.0-1.0, "reasoning": "FACTS: ... | ASSUMPTIONS: ... | THESIS: ... | VALUATION/EVIDENCE: ... | WHY NOW: ... | INVALIDATION: ... | RISK: ...", "stop_loss_pct": 0.05, "take_profit_pct": 0.15, "exit_when": "", "max_hold_days": 0}], "read_articles": ["url1"], "search_queries": ["query1"]}
...
exit_when: free-text condition for exit (e.g. "exit if BTC breaks 50k support").
max_hold_days: optional. Set to a specific number of days if the thesis is time-bound (e.g. ETF approval in 21 days -> max_hold_days: 25). Set to 0 (default) for thesis-driven holds with no fixed time limit. Only set a limit when the trade thesis has a clear expiration.
...
- High conviction (>0.7): up to 15% of NAV
...
class CryptoPMAgent(BasePodAgent):
    ...
    async def _web_search_and_decide(
        ...
        try:
            ...
            logger.warning("[crypto.pm] Web search re-prompt failed: %s", e)
            return (first_decision, None)
    ...
    async def _llm_decision(self, features: dict, sizing_context: dict | None = None,
        ...
        user_content += tradeable_entry_thesis_instruction("crypto")
        ...
        try:
            ...
            for t in trades[:3]:
                ...
                logger.info("[crypto.pm] LLM: %s %s %s conv=%.1f (%s)%s", action, symbol, qty, conviction, reasoning, log_suffix)
