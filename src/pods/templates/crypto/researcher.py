...
class CryptoResearcher(BasePodAgent):
    ...
    async def run_cycle(self, context: dict) -> dict:
        ...
        scored_headlines, scored_preds = score_items(headline_dicts, pred_dicts, "crypto")
        ...
        try:
            ...
            for h in candidates[:2]:
                ...
                results = await self._web_searcher.search(
                    f"{title} cryptocurrency market analysis {now.strftime('%B %Y')}", max_results=3
                )
                if results:
                    top = results[0]
            ...
            if deep_dives:
                ...
                logger.info("[crypto.researcher] %d headline deep-dives fetched", len(deep_dives))

            # Macro outlook search
            macro_outlook = self._ns.get("features", {}).get("macro_outlook", "") if self._ns.get("features") else ""
            if macro_outlook:
                results = await self._web_searcher.search(f"cryptocurrency market {macro_outlook} outlook {now.strftime('%B %Y')}")
                self._ns.set("web_search_results", results)
        except Exception as e:
            logger.debug("[crypto.researcher] Web search skipped: %s", e)
        ...
        try:
            ...
            logger.debug("[crypto.researcher] Events calendar skipped: %s", e)
