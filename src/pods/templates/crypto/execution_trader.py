...
class CryptoExecutionTrader(BasePodAgent):
    ...
    async def run_cycle(self, context: dict) -> dict:
        ...
        if token is None or not token.is_valid():
            logger.warning(
                "[crypto.exec] Invalid/expired token — order %s rejected", order.id
            )
        ...
        if risk_halt:
            ...
            logger.error(
                "[crypto.exec] Order %s rejected: Risk halt active (%s)", order.id, risk_halt_reason
            )
        ...
        if drawdown_halt and order.side == Side.BUY:
            logger.error("[crypto.exec] Order %s rejected: firm drawdown halt (no new buys)", order.id)
            return {
                "execution_rejected": True,
    ...
    async def _execute_via_alpaca(
        ...
        try:
            ...
            if mandate and self._pod_id in mandate.pod_allocations:
                ...
                if current_notional + requested_notional > max_notional:
                    ...
                    if available_notional <= 0:
                        logger.warning(
                            "[crypto.exec] Order %s rejected: allocation limit reached (%.0f%%, current=$%.2f, max=$%.2f)",
                    ...
                    logger.info(
                        "[crypto.exec] Scaling order %s from %.2f to %.2f (allocation limit)",
            ...
            if token and hasattr(token, "constraints") and token.constraints:
                ...
                if new_leverage > max_leverage:
                    logger.warning(
                        "[crypto.exec] Order %s rejected: leverage limit (%.1fx) exceeded (current+requested=%.1fx)",
        ...
        try:
            logger.info(
                "[crypto.exec] Submitting order to Alpaca: %s %.0f %s @ $%s",
            ...
            if result.status in ("FILLED", "PARTIAL"):
                ...
                try:
                    ...
                    logger.debug("[crypto.exec] Failed to publish fill event: %s", e)
            ...
            logger.error("[crypto.exec] Order execution failed: %s", exc)
            return {
    ...
    def _queue_for_paper_adapter(self, order: Order) -> dict:
        ...
        logger.info(
            "[crypto.exec] Order queued (paper mode): %s %.0f %s",
