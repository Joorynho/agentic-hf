...
class BetaExecutionTrader(BasePodAgent):
    ...
    async def run_cycle(self, context: dict) -> dict:
        ...
        if risk_halt:
            ...
            logger.error(
                "[beta.exec] Order %s rejected: Risk halt active (%s)", order.id, risk_halt_reason
            )
        ...
        try:
            ...
            if mandate and self._pod_id in mandate.pod_allocations:
                ...
                if current_notional + requested_notional > max_notional:
                    ...
                    if available_notional <= 0:
                        logger.warning(
                            "[beta.exec] Order %s rejected: allocation limit reached (%.0f%%, current=$%.2f, max=$%.2f)",
                    ...
                    logger.info(
                        "[beta.exec] Scaling order %s from %.2f to %.2f (allocation limit)",
            ...
            if token and hasattr(token, "constraints") and token.constraints:
                ...
                if new_leverage > max_leverage:
                    logger.warning(
                        "[beta.exec] Order %s rejected: leverage limit (%.1fx) exceeded (current+requested=%.1fx)",
            ...
            logger.error("[beta.exec] Error checking governance constraints: %s", e)
            # Continue with execution (fail-open for allocation checks)

        try:
            logger.info(
                "[beta.exec] Submitting order to Alpaca: %s %.0f %s @ $%s",
            ...
            if result.status in ("FILLED", "PARTIAL"):
                ...
                try:
                    ...
                    logger.debug("[beta.exec] Failed to publish fill event: %s", e)
            ...
            logger.error("[beta.exec] Order execution failed: %s", exc)
            return {
    ...
    def _queue_for_paper_adapter(self, order: Order) -> dict:
        ...
        logger.info(
            "[beta.exec] Order queued (paper mode): %s %.0f %s",
