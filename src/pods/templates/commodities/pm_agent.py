...
_COMMODITIES_SYSTEM = """You are a senior commodities portfolio manager at an institutional macro hedge fund.
You receive real macro data, prediction market odds, and news headlines every cycle.
Your job is to produce institutional-quality commodity trade ideas with deep, differentiated reasoning.
...
- Do NOT chase commodity spikes. If oil already surged on a headline, the move is priced.
- Instead: WHY did it spike? What's the structural driver? Which related commodities
  (substitutes, downstream products, producers) haven't repriced yet?
- Example: "Oil surged on OPEC supply cut" → look at natural gas (substitution effect),
  oil services (XOP, OIH), or inflation-sensitive assets (TIPS, gold) that lag.
- Supply disruption news: think about second-order effects on transportation, agriculture,
  or manufacturing costs.
- Do NOT restate headlines. Explain the IMPLICATION across the commodity complex.

REASONING STANDARD — every trade MUST include:
- THESIS: The macro setup (inflation, real rates, supply/demand) and why it favors this commodity
- EDGE: What is the market not pricing? What supply/demand dynamic is underappreciated?
- SECOND-ORDER: Which related commodities or producers benefit from the same dynamic but haven't moved?
- RISK: What would invalidate? (OPEC reversal, demand destruction, weather normalization, policy change)
- TIMING: Why now? Is there a seasonal pattern, inventory report, or geopolitical deadline?

ANTI-PATTERNS (will get your trades rejected):
- "Oil spiked so buy oil" — chasing is not investing
- Generic inflation commentary without naming specific trades
...
WEB SEARCH:
If you need information not provided (e.g. OPEC meeting outcome, crop report dates),
...
class CommoditiesPMAgent(BasePodAgent):
    ...
    async def _evaluate_risk_revision(self, context: dict) -> dict:
        ...
        try:
            ...
            if accept:
                logger.info("[commodities.pm] Accepted revision: %s %s %.4f (%s)", revised_order.side.value, revised_order.symbol, revised_order.quantity, reasoning)
                return {"order": revised_order}
            else:
                logger.info("[commodities.pm] Rejected revision: %s %s (%s)", revised_order.side.value, revised_order.symbol, reasoning)
                return {}
        except Exception as e:
            logger.warning("[commodities.pm] Revision eval failed, accepting: %s", e)
    ...
    async def _web_search_and_decide(
        ...
        try:
            ...
            logger.warning("[commodities.pm] Web search re-prompt failed: %s", e)
            return (first_decision, None)
    ...
    async def _llm_decision(self, features: dict, sizing_context: dict | None = None,
        ...
        if upcoming_events:
            ...
            sections.append("  WARNING: Binary events create unpredictable gaps. Reduce size or avoid.")
        ...
        try:
            ...
            for t in trades[:3]:
                ...
                if qty <= 0:
                    logger.info("[commodities.pm] Skipped %s %s: %s", action, symbol, clamp_note)
                    continue
                est_price = self._estimate_price(symbol)
                ...
                logger.info("[commodities.pm] LLM: %s %s %.2f conv=%.1f (%s)%s", action, symbol, qty, conviction, reasoning, log_suffix)
